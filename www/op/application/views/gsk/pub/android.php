...
<form method="post" action="<?php echo site_url('gsk_pub/do_pub_android');?>"  name="form_id_do_pub_android" >
<!-- Modal -->
<div id="modal_pub_android" method="post" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
...
$version_channel[$item['f_android_version']] = $version_channel[$item['f_android_version']] . $item['f_id'] . ' ' .  $item['f_name'] . '  '; 
endforeach;?>
        ...
        <div class="panel panel-info panel-dark widget-profile">
            <div class="panel-heading">
                ...
                <div class="widget-profile-header">
                    <span><i class="fa fa-android"></i> 选择版本</span>
                </div>
            </div> <!-- / .panel-heading -->
            <div class="list-group">
                <?php foreach( $versions as $item ) :?>
                    <p class="list-group-item">
                        <input type="radio" name="radio_pub_android" id=<?=$item['f_version']?> value=<?=$item['f_version']?>>
    ...
    <div class="col-sm-4 col-sm-pull-8">
        <div class="panel panel-success panel-dark widget-profile">
            <div class="panel-heading">
                <div class="widget-profile-bg-icon"><i class="fa fa-android"></i></div>
                <div class="widget-profile-header">
                    <span><i class="fa fa-android"></i> 选择渠道</span>
                </div>
            </div> <!-- / .panel-heading -->
            <div class="list-group">
                ...
                <?php if($item['f_os'] == 0 || $item['f_os'] == 3) :?>
                    <p class="list-group-item">
                        <input type="checkbox" id="<?=$item['f_name']?>" name="checkbox_pub_android[]"  value=<?=$item['f_id']?> />
                        <?=$item['f_name']?><span>[<?=$item['f_id']?>]</span>
                        <span class="pull-right">
                        <a target="_blank" href="<?php echo site_url('gsk_pub/pub_list?type=android&c=');?><?=$item['f_id']?>"><span class="label label-info">历史版本</span></a>
                        ...
                        <a target="_blank" href="http://zy.glodon.com/util/info.php?c=<?=$item['f_id']?>&clt=3"><i class="fa fa-cog"></i></a>
                        <span class="label label-<?=get_random_state($item['f_android_version']);?>"><?=$item['f_android_version'] ?></span>
                        </span>
                    </p>
                <?php endif;?>
                <?php endforeach ?>
        ...
        <div class="panel panel-dark">
            <div class="panel-body text-center">
                <a href="#" class="btn btn-lg btn-primary btn-flat" data-toggle="modal" data-target="#modal_pub_android">发  布</a>
            </div>
        </div>
    </div>
...
$(document).ready(function(){
        $("#modal_pub_android").on('show.bs.modal', function (e) {
            var select_ver = $( "input[type=radio][name='radio_pub_android']:checked" ).val()
            ...
            var select_channel = ""
            $( "input[type=checkbox][name='checkbox_pub_android[]']:checked" ).each(function(){ 
                select_channel += "<span style='font-size:16px;-webkit-text-stroke: 1px #000000;'>" + this.value + "</span>" + this.id + "      "
                warning_msg = "渠道:"+this.id+ " 版本:" + select_ver +" 不存在于OSS，请检查！"
                $.get("<?php echo site_url('gsk_pub/do_check_oss?channel=');?>"+this.value+"&version="+select_ver, function(data, status){
                    if( data != 200 ){
                        ( warning_msg );
                        return e.preventDefault() 
                    }
