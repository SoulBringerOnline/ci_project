...
<div class="row">
	<div class="col-md-12" style="margin-bottom:16px;">
		<form action="<?php echo site_url('gsk_user/edit_user');?>" method="POST" class="form-horizontal" enctype="multipart/form-data">
			<div class="form-group">
				<label for="msgflag" class="col-sm-2 control-label">用户glodon账号:</label>
				<div class="col-sm-8">
					<input type="text" class="form-control" id="account" name="account" value="<?=$data['account']?>"  placeholder="用户的glodon账号">
				</div>
			</div>
			<div class="form-group">
				<label for="baseid" class="col-sm-2 control-label">权限标记位:</label>
				<div class="col-sm-8">
					<input type="text" class="form-control" id="priv" name="priv" value="<?=$data['priv']?>"  placeholder="权限level，说明">
				</div>
			</div>
			<div class="form-group">
				<label for="begintime" class="col-sm-2 control-label">角色群组</label>
				<div class="col-sm-8">
					<input type="text" class="form-control" id="group" name="group" value="<?=$data['group']?>"  placeholder="用户的角色群组 1: 管理员  2：运营   3： 产品  4：开发">
				</div>
			</div>
			<div class="form-group">
				<label for="finishtime" class="col-sm-2 control-label">用户状态</label>
				<div class="col-sm-8">
					<input type="text" class="form-control" id="status" name="status" value="<?=$data['status']?>" placeholder="用户状态，0正常 -1 封禁">
				</div>
			</div>
...
<script>
	btn.onclick = function() {
		...
		$.ajax({
			type:"POST",
			url:"<?=site_url('gsk_user_op/do_edit_user');?>",
			data:data,
			datatype: "json",
