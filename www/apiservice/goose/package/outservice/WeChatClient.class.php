<?PHP
	 ...
	 */
	namespace Goose\Package\Outservice;
	use \Goose\Package\Outservice\WechatHongbao;
	...
	class WeChatClient{
		...
		public function __construct( $appid, $appsecret , $app_mchid="1295949501"){
			$this->_appid     = $appid;
			$this->_appsecret = $appsecret;
			$this->_app_mchid = $app_mchid;
		}
		...
		public function pay($re_openid) {
			$wxHongBaoHelper = new WechatHongbao();

			$wxHongBaoHelper->setParameter("nonce_str", $this->great_rand());//随机字符串，丌长于 32 位
			$wxHongBaoHelper->setParameter("mch_billno", $this->_app_mchid.date('YmdHis').rand(1000, 9999));//订单号
			$wxHongBaoHelper->setParameter("mch_id", $this->_app_mchid);//商户号
			$wxHongBaoHelper->setParameter("wxappid", $this->_appid);
